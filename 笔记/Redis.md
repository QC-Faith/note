...
缓存击穿是指<font color='Apricot'>**缓存中没有但数据库中有**</font>的数据（一般是缓存时间到期），一个`key`非常热点，并发访问特别多，在它失效的瞬间，持续的大并发就穿破缓存，直接请求到了数据库。
...
缓存雪崩是指在某个时间点，大量的缓存数据同时过期或失效，导致大量请求直接访问数据库。<font color='Apricot'>**和缓存击穿不同的是，缓存击穿指并发查同一条数据，缓存雪崩是不同数据都过期了，很多数据都查不到从而查数据库**</font>。
